...
namespace fyx {
	...
	namespace cpu {
		...
		inline bool has_avx512() {
			return get_features().avx512f && get_features().avx512dq;
		}
		inline bool has_avx2() {
			return get_features().avx2;
		}
		inline bool has_sse42() {
			return get_features().sse42;
		}
	}
	// ═══════════════════════════════════════════════════════════════════════════
	// 第四部分: 配置参数
	...
	namespace config {
		...
		// 内存安全阈值
		inline constexpr size_t MAX_ALLOC_SIZE = size_t(1) << 40;
		// 1TB
		// 分层阈值
		inline constexpr size_t HIERARCHICAL_L1_THRESHOLD = 8000;
		// L1缓存友好
		inline constexpr size_t HIERARCHICAL_L2_THRESHOLD = 100000;
		// L2缓存友好
		inline constexpr size_t HIERARCHICAL_L3_THRESHOLD = 1000000;
		// L3缓存友好
		...
		inline std::atomic<size_t>& max_memory_limit() {
			static std::atomic<size_t> limit {
				L3_SIZE * 8
			}
			;
			// 默认64MB
	...
	struct Options {
		...
		static Options stable_sort() {
			Options o;
			o.stable = true;
			return o;
		}
		static Options radix_only() {
			Options o;
			o.force_radix = true;
			return o;
		}
		static Options comparison_only() {
			Options o;
			o.force_comparison = true;
			return o;
		}
	...
	namespace detail {
		...
		namespace mem {
			...
			inline void* aligned_alloc(size_t size, size_t align = config::CACHE_LINE) noexcept {
				...
				if (align == 0 || (align & (align - 1)) != 0) align = config::CACHE_LINE;
				...
				# else
				void* p = nullptr;
				int ret = posix_memalign(&p, align, aligned_size);
				if (ret != 0) {
					return nullptr;
				}
				return p;
			...
			class alignas(config::CACHE_LINE) Buffer {
				...
				Buffer() = default;
				explicit Buffer(size_t n) : len(n) {
					if (n == 0) {
				...
				: ptr(o.ptr), len(o.len), status(o.status) {
					o.ptr = nullptr;
					o.len = 0;
				...
				bool resize(size_t n) {
					...
					if (ptr) {
						...
						// 【修复#4】: 保留旧数据
						if (old_ptr) {
							size_t copy_len = std::min(old_len, n);
							for (size_t i = 0; i < copy_len; ++i) {
								ptr[i] = std::move(old_ptr[i]);
							}
		...
		namespace simd_tables {
			...
			struct alignas(64) PrecomputedTables {
				...
				__m512i rev_idx_8;
				// 常用blend掩码
				__mmask16 blend_AAAA;
				__mmask16 blend_CCCC;
				__mmask16 blend_F0F0;
				__mmask16 blend_FF00;
				__mmask8 blend_AA;
				__mmask8 blend_CC;
				__mmask8 blend_F0;
				PrecomputedTables() {
			...
			;
			// 【修复#2】: 条件编译，确保函数仅在AVX512启用时存在
			#ifdef FYX_AVX512
			inline const PrecomputedTables& get_tables() {
		...
		namespace simd512 {
			...
			FYX_INLINE __m512i sort_16xu32(__m512i v) noexcept {
				...
				const auto& tbl = get_tables();
				t = _mm512_shuffle_epi32(v, _MM_PERM_CDAB);
				mn = _mm512_min_epu32(v, t);
				mx = _mm512_max_epu32(v, t);
				v = _mm512_mask_blend_epi32(tbl.blend_AAAA, mn, mx);
				t = _mm512_shuffle_epi32(v, _MM_PERM_BADC);
				mn = _mm512_min_epu32(v, t);
				mx = _mm512_max_epu32(v, t);
		...
		namespace simd {
			...
			FYX_INLINE void sort_small_simd(T* arr, size_t n) noexcept {
				if constexpr (std::is_same_v<T, int32_t>) {
					#ifdef FYX_AVX512
					if (n == 128) {
						simd512::sort_128xi32(arr);
						return;
					}
					if (n == 64) {
						simd512::sort_64xi32(arr);
						return;
					}
					if (n == 32) {
						simd512::sort_32xi32(arr);
						return;
					}
					if (n == 16) {
					...
					#ifdef FYX_AVX2
					if (n == 32) {
						simd256::sort_32xi32(arr);
						return;
					}
					if (n == 16) {
						simd256::sort_16xi32(arr);
						return;
					}
					if (n == 8) {
		...
		namespace counting {
			...
			bool try_sort(T* a, size_t n) noexcept {
				if constexpr (!std::is_integral_v<T>) return false;
				...
				if constexpr (std::is_signed_v<T>) {
					// 对于有符号类型，需要特别处理
					int64_t diff = static_cast<int64_t>(true_max) - static_cast<int64_t>(true_min);
					if (diff < 0) return false;
					// 溢出检查
					range64 = static_cast<uint64_t>(diff);
				} else {
				...
				for (size_t i = 0; i < n; ++i) {
					size_t idx = static_cast<size_t>(static_cast<U>(a[i]) - static_cast<U>(true_min));
					++count[idx];
				}
				size_t pos = 0;
				for (size_t i = 0; i < range; ++i) {
					T val = static_cast<T>(static_cast<U>(true_min) + static_cast<U>(i));
					size_t c = count[i];
					while (c--) a[pos++] = val;
				}
				return true;
		...
		namespace radix {
			...
			FYX_NOINLINE void sort_l1_block(T* a, size_t n) {
				...
				alignas(64) T buffer[config::HIERARCHICAL_L1_THRESHOLD];
			...
			FYX_NOINLINE void american_flag_sort(T* a, size_t n, int shift = -1) {
				...
				if (shift >= 8) {
					...
					for (size_t b = 0; b < NB; ++b) {
						if (count[b] > 1) {
							american_flag_sort(a + pos, count[b], shift - 8);
						}
						pos += count[b];
					}
				}
		...
		namespace parallel {
			...
			class LockFreeWorkStealQueue {
				...
				struct CircularArray {
					std::atomic<WorkItem>* items;
					size_t mask;
					CircularArray(size_t cap) : mask(cap - 1) {
						items = new std::atomic<WorkItem>[cap];
					}
					~CircularArray() {
						delete[] items;
					}
					...
					// 【修复#3】: 修复 grow 函数的并发安全问题
					CircularArray* grow(size_t bottom, size_t top) {
						CircularArray* old_arr = array.load(std::memory_order_acquire);
						CircularArray* new_arr = new CircularArray(old_arr->capacity() * 2);
				...
				;
				alignas(config::CACHE_LINE) std::atomic<size_t> bottom {
					0
				}
				;
				alignas(config::CACHE_LINE) std::atomic<size_t> top {
					0
				}
				;
				alignas(config::CACHE_LINE) std::atomic<CircularArray*> array;
				// 垃圾收集
				std::vector<CircularArray*> garbage;
				...
				void push(WorkItem item) {
					...
					if (b - t >= arr->capacity() - 1) {
						// 需要扩容
						CircularArray* new_arr = arr->grow(b, t); {
							std::lock_guard<std::mutex> lock(garbage_mutex);
							garbage.push_back(arr);
						}
					...
					while (!done.load(std::memory_order_relaxed)) {
						...
						if (queues[tid].pop(item)) {
							...
							if (item.length > config::WORK_STEAL_THRESHOLD && item.depth < 3) {
								...
								WorkItem right = {
									data, item.start + mid, item.length - mid, item.depth + 1
								}
								;
			...
			DataProfile analyze(const T* a, size_t n, Cmp& cmp) {
				...
				for (size_t i = 0; i < sample_size; ++i) {
					...
					if constexpr (std::is_integral_v<T>) {
						++bucket_counts[(static_cast<size_t>(a[idx]) >> 4) & 0xF];
					}
				}
				...
				profile.is_nearly_sorted = (asc > sample_size * 9 / 10) ||
				(desc > sample_size * 9 / 10);
				profile.has_many_duplicates = eq > sample_size / 4;
				// 计算熵
				double total = static_cast<double>(sample_size);
				...
				// 检查是否小范围
				if constexpr (std::is_integral_v<T>) {
					auto [mn, mx] = simd::find_minmax(a, std::min(n, size_t(1000)));
					uint64_t range = static_cast<uint64_t>(mx) - static_cast<uint64_t>(mn);
					profile.is_small_range = range < n * config::COUNTING_SORT_RATIO;
				}
				return profile;
			}
			// 根据数据特征选择最优算法
			...
			struct Classifier {
				...
				FYX_INLINE size_t classify(const T& val) const noexcept {
					...
					while (lo < hi) {
						...
						if (cmp(pivots[mid], val)) lo = mid + 1; else hi = mid;
					}
					return lo;
				}
		...
		static void sort(T* a, size_t n, Cmp cmp = Cmp(), const Options& opts = Options::defaults()) {
			...
			if (profile.is_reverse) {
				...
				for (size_t i = 1; i < n && sorted; ++i) {
					if (cmp(a[i], a[i-1])) sorted = false;
				}
				if (sorted) return;
				// 不是真正逆序，继续排序
			}
			// 根据选项和数据特征选择算法
			auto algo = detail::adaptive::select_algorithm<T, Cmp>(n, profile, opts);
			// 强制比较排序
			if (opts.force_comparison) {
				#if FYX_ENABLE_PARALLEL
			...
			if (opts.stable) {
				...
				detail::indirect::stable_sort(a, n, cmp); else
	...
	void sort(Container& c, const Options& opts = Options::defaults()) {
		...
		Sorter<T>::sort(c.data(), c.size(), std::less<T> {
		}
		, opts); else {
			std::vector<T> tmp(c.begin(), c.end());
			Sorter<T>::sort(tmp.data(), tmp.size(), std::less<T> {
			}
			, opts);
	...
	void sort(Container& c, Cmp cmp, const Options& opts = Options::defaults()) {
		...
		if (c.size() < 2) return;
		if constexpr (detail::traits::is_contiguous_v<Container>)
		Sorter<T>::sort(c.data(), c.size(), cmp, opts); else {
			std::vector<T> tmp(c.begin(), c.end());
	...
	void sort(It first, It last, const Options& opts = Options::defaults()) {
		...
		if constexpr (detail::iter_traits::is_contiguous_v<It> || std::is_pointer_v<It>) {
			Sorter<T>::sort(&(*first), static_cast<size_t>(n), std::less<T> {
			}
			, opts);
	...
	void stable_sort(Container& c, const Options& opts = Options::defaults()) {
		...
		if (c.size() < 2) return;
		if constexpr (detail::traits::is_contiguous_v<Container>)
		Sorter<T>::stable_sort(c.data(), c.size(), std::less<T> {
		}
		, opts); else {
			std::vector<T> tmp(c.begin(), c.end());
			Sorter<T>::stable_sort(tmp.data(), tmp.size(), std::less<T> {
			}
			, opts);
			std::copy(tmp.begin(), tmp.end(), c.begin());
		}
	...
	void stable_sort(Container& c, Cmp cmp, const Options& opts = Options::defaults()) {
		...
		if (c.size() < 2) return;
		if constexpr (detail::traits::is_contiguous_v<Container>)
		Sorter<T>::stable_sort(c.data(), c.size(), cmp, opts); else {
			std::vector<T> tmp(c.begin(), c.end());
	...
	void stable_sort(It first, It last, const Options& opts = Options::defaults()) {
		...
		if constexpr (std::is_pointer_v<It>) {
			Sorter<T>::stable_sort(first, static_cast<size_t>(n), std::less<T> {
			}
			, opts);
		} else {
			std::vector<T> tmp(first, last);
	...
	inline int version_major() {
		return FYX_VERSION_MAJOR;
	}
	inline int version_minor() {
		return FYX_VERSION_MINOR;
	}
	inline int version_patch() {
		return FYX_VERSION_PATCH;
	}
}
// namespace fyx
...
╚═══════════════════════════════════════════════════════════════════════════════╝
	...
	test("稳定排序", [&]() {
		...
		;
		std::vector<S> a(5000);
		for (int i = 0; i < 5000; ++i) a[i] = {
			static_cast<int>(rng() % 100), i
		}
		;
		fyx::stable_sort(a);

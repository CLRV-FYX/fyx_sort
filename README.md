...
高熵数值默认顺序在 3+ worker 上回到 chunked parallel radix（保留全 pass 快速规划、32-bit/value-buffer 优化），避免 4H8G 上 MSD 分桶开销反噬；只有 2-worker/bandwidth-constrained 环境才启用 `float` top-byte / 64-bit top16 MSD-bucket hybrid；

...
低基数 radix-key 场景走并行 sparse counting；整数 64..65536 小值域会优先走 sample-gated 并行 dense-range count/fill，浮点/64-bit 稀疏低基数会先尝试 collision-free rank16 direct-map，浮点小值域也保留 compact prefix direct-map，以避免逐元素 hash probe；通用大输入走并行 sample sort（并行分类、散射和桶递归）；其它中等输入保留任务并行分治，

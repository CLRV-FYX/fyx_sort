...
- Custom comparators that sample as natural ascending/descending order now safely recover the radix/counting fast paths for arithmetic types, the high-prefix radix path for high-entropy 64-bit numerics, and the MSD/value-count paths for `std::string`; every guarded fast path is verified with `std::is_sorted(comp)` before returning, otherwise the comparison sorter continues from the resulting permutation.

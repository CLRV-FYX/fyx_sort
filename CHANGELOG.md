...
- Sample-sort classification now uses an unrolled fixed-256 Eytzinger descent; serial sample-sort scatter uses a single prefix-position pass instead of rebuilding per-block prefix tables, and low-distinct arithmetic samples fall back to pdqsort before paying the bucket-scatter cost.  The comparison recursion threshold was lowered to keep high-distinct buckets in sample-sort longer when that is cheaper than large pdqsort leaves.

...
- Added an MSD-bucket hybrid for the remaining vqsort numeric gaps: high-entropy `float` first splits on the top radix byte and sorts buckets with a 3-pass lower radix; high-entropy 64-bit integer/`double` splits on the top 16 radix bits, then uses tiny-bucket PDQ or 6-pass lower radix inside buckets.  This trades one global partition for much less full-array random scatter traffic on 10M random numeric inputs.

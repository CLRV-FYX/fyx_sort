...
├── §10b AVX-512 向量快排               compress-store 分区 + 原生类型双调网络叶子（高基数数值）
...
### 2.0 高基数数值：AVX-512 向量快排（§10b）

LSD 基数排序对随机 32 位键至少要三趟，一趟约 3.1 ns/elem（`tools/dev/NOTES.md`
里把这条下界算死了：更宽的数字、缓存分块、更深的写合并缓冲、向量化 scatter
全都实测输了）。所以高基数、宽值域的数值输入改走一条不按位分趟的路：

- **分区**：`vpcompressd` / `vcompressps` 原地两端写。一次比较得到掩码，
  两条 masked compress-store 分别把「小于枢轴」和「不小于枢轴」写到空隙的两端，
  任何元素只写一次。
- **每轮 4 个向量，且读哪一侧不用分支**：读左还是读右取决于数据怎么分，
  一向量一判断时这个分支基本每隔一次就预测错，代价比分区本身还大
  （1M 随机 int32：0.0060 s → 0.0048 s，只改这一点）。
- **叶子**：最多 16 个向量（4 字节类型 256 元素、8 字节 128 元素）直接跑
  §7 的 Batcher 双调网络，但策略走**原生类型**的 min/max，不再 encode 成
  radix key —— 快排内部的比较本来就在值域里，encode/decode 两趟是纯损失。
- **退化枢轴**：两路分区在「枢轴等于区间最小值」时低侧为空、不前进。
  这种区间会用严格比较（`> pivot`）再分一次，把等于枢轴的那一块（至少一个元素，
  且已经在最终位置）挪到前面，保证每轮都缩短。
- **浮点总序**：硬件比较区分不了 `-0` 和 `+0`，也排不了 NaN。入口先做一次只读
  SIMD 扫描，发现任一个就让位给 radix，那条路本来就保证 `-NaN < -0 < +0 < +NaN`。
- **并行**：前几层分区在原地做完后，两侧作为任务交给 §10 的线程池；
  单次分区本身是顺序的（并行分区要第二个数组和一轮前缀和，两个 worker 上不划算）。
- **不适用的类型**：64 位整数仍旧走 high-prefix radix —— 它两趟排完 24–26 位前缀
  再修 tie，比快排的 log(n/leaf) 层便宜（8M 随机 int64：0.089 s vs 0.096 s）。
